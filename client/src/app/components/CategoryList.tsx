  ...
  const CategoryItem = ({ category, isSelected, children, theme }: CategoryItem) => {
    ...
    return (
      <Link to={`/catalog/categories/${category.id}`} style={{ textDecoration: 'none' }}>
        <Box sx={{
            borderRadius: '10px', backgroundColor: isSelected ? theme.palette.mode === 'dark' ? 'grey.900' : 'primary.main' : 'transparent',}}>
        ...
        <motion.div
          ...
          style={{
            ...
            cursor: 'pointer',
            userSelect: 'none',
            color: isSelected ?  theme.palette.mode === 'dark' ? theme.palette.brandSecondary.light : theme.palette.warning.light : theme.palette.mode,
